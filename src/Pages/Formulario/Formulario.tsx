...
import { useSetMembro } from "../../Hooks/useSetMembro";


const Formulario = () => {

  const { setMembroFirestore, loading, error } = useSetMembro()
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const id = await setMembroFirestore(membro)
  ...
  return (
    <div className={style.container}>
      <form onSubmit={handleSubmit} className={style.form}>
        ...
        <button>{loading ? 'Salvando' : "Enviar Formulario"}</button>
        {error && <p>{error}</p>}
      </form >
    </div >
  )
